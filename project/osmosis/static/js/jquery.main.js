...
function initEditTable() {
	var removeModal = jQuery('#deleteModal');
	var editModal = jQuery('#editModal');
	var currRow = null;

	jQuery('.table').each(function() {
		var table = jQuery(this);
		var btnsRemove = table.find('.delete');
		var btnsEdit = table.find('.edit');

		btnsRemove.on('click', function(e) {
			e.preventDefault();
			currRow = jQuery(this).closest('tr');
	...
	if (removeModal.length) {
		var btnRemove = removeModal.find('.btn-remove');
	...
	if (editModal.length) {
		var btnSave = editModal.find('.btn-save');
		var currNameField = editModal.find('#user-name');
		var currAmountField = editModal.find('#total-amount');
		var currStateField = editModal.find('#status');
		var stateOptions = currStateField.find('option');
		var editForm = editModal.find('.payout-form');

		btnSave.on('click touchstart', function(e) {
			e.preventDefault();
			var currName = currRow.find('.name');
			var currAmount = currRow.find('.amount');
			var currState = currRow.find('.status');
			var amountValue = currAmountField.val();

			if (currNameField.val() !== '') {
				currName.text(currNameField.val());
			}

			if (amountValue !== '') {
				currAmount.text(formatMoney(amountValue));
			}

			if (currStateField.prop('selectedIndex') !== 0) {
				currState
					.text(currStateField.val())
					.removeAttr('class')
					.addClass('status badge badge-' + stateOptions.eq(currStateField.prop('selectedIndex')).data('value'));
			}
